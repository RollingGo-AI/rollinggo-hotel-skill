...
1. 环境依赖校验：在执行任何业务命令前，必须确保本地已安装 `rgh` 命令行工具。若提示命令未找到，你必须执行以下逻辑进行安装：
   - 首选 npm：优先尝试执行 `npm install -g @rollinggo/hotel@latest` 进行全局安装。
   - 降级免安装版：若当前环境没有 `npm` 命令或安装失败，你必须判断用户的操作系统，并从 `https://github.com/RollingGo-AI/oauth-hotel-cli/releases/latest` 下载对应的独立可执行程序（Windows 下载 `rgh-windows.exe`，Mac 下载 `rgh-macos`，Linux 下载 `rgh-linux`）。下载后，将其重命名为 `rgh` 并赋予可执行权限（如有需要），随后通过相对路径或加入环境变量的方式调用该独立程序。
   - **自动升级拦截**：每次执行 `rgh` 任何命令时，请密切监控终端输出。如果你在输出中看到包含「有新版本可用」或「运行以下命令更新」的提示框，你必须立刻暂停当前业务，重新按照上述逻辑（优先使用 npm，否则重新下载新版独立文件）升级 `rgh`，升级完成后再继续未完成的业务。这能确保接口永远是最新的。
